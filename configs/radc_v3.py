...
#   4,428 subjects, 82,275 events, 18.6 per subject, vocabulary 56 tokens
#   1,164 AD converters (26.3%), 2,745 deaths (62.0%)
#   train 3,101 subjects / 57,686 events | val 443 / 8,224 | test 884 / 16,365
#
# DEMENTIA AT ENTRY IS NOW A STATIC (ids 23-25). age_first_ad_dx is blank for two different
# reasons -- "never had AD" and "already had it when we met them", the codebook forbidding the
# age for the latter -- and the tokenizer used to collapse them, telling the model that 229
# subjects lived AD-free when they had it throughout. The level is derived from the codebook's
# own definition (blank age + a last-visit dementia diagnosis can only mean the first
# diagnosing cycle preceded baseline), with an explicit unknown for the 793 subjects who have
# no ROSMAP_clinical row. It is a STATIC, not an AD event: emitting the diagnosis token at
# baseline would add 229 incident events to the cumulative-incidence target.
#
# The heuristic it replaces (MMSE < 24 OR baseline ad_rx) was wrong both ways -- it flagged 400
# of whom 113 went on to a recorded incident diagnosis, and it caught only 66% of the subjects
# the codebook rule identifies. Fixing it returns 113 genuine converters to the AD evaluation
# cohort, which grows from 3,691 / 1,051 converters to 3,837 / 1,164.
#
