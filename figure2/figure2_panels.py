...
# One colour per cognitive stage, light -> dark with severity, plus a neutral grey for Death.
# Sized from the state space so switching radc_states.STAGE_MODE to "mmse7" does not silently
# reuse stage colours modulo 4 -- the palette-cycling bug the figure audit caught elsewhere.
STATE_COLORS = ps.severity_ramp(S.NSTAGE) + [ps.REF_GREY]
TRAJ_COLORS = {
    "Stable": "#0072B2", "Improved": "#009E73", "Progressed (stage)": "#E69F00",
...
def compute_A_time(cache):
    ...
    df, grids = cache["df"], cache["grids"]
    b = df["baseline_state"].to_numpy()
    fp = grids["fp"]                                     # (N, NSLOT, n_mc), years after baseline
    obs_all = np.stack([df[f"obs_t_{nm}"].to_numpy(float) for nm in F2.ALL_NAMES], 1)   # (N, NSLOT)
    n = len(df)
    obs = np.full(n, np.nan); pred = np.full(n, np.nan); n_used = np.zeros(n, int)
    for i in range(n):
        # every cognitive stage but the current one. NSTAGE/DEATH_IDX rather than the NACC
        # literals 4 and 4: those two happen to coincide for the four Folstein stages, so a
        # hardcoded 4 would keep "working" and then silently mis-slice under STAGE_MODE="mmse7".
        cog = [s for s in range(S.NSTAGE) if s != b[i]]
        o = float(np.min(obs_all[i, cog])); d = obs_all[i, F2.DEATH_IDX]
        if not np.isfinite(o) or (np.isfinite(d) and d < o):
            continue
        t_sim = fp[i, cog, :].min(0)
        ok = np.isfinite(t_sim) & (t_sim <= fp[i, F2.DEATH_IDX, :])
        if not ok.any():
            continue
...
def endpoints_B(df, grids):
    b = df["baseline_state"].to_numpy()
    return [
        # from radc_states.ENDPOINTS: the cognitive threshold, the diagnosis, and death --
        # the same three roles the NACC figure used, with AD standing in for the "Dementia"
...
def compute_B(cache):
    ...
    # Give the frames their columns even when no row qualified. Without this, a cohort too thin
    # to fill any (endpoint, time-bin) cell yields a column-less DataFrame and panel_B dies on
    # `err["endpoint"]` with a KeyError -- a crash rather than the "not enough data" the rest of
    # this module is careful to report. Inherited from the NACC version; it never fired there
    # because that cohort was 4,589 subjects, but it fires on any --limit smoke run.
    err_cols = ["endpoint", "bin", "bin_lo", "bin_hi", "n", "mae", "mae_lo", "mae_hi",
                "bias", "median_bias", "naive_mae", "errors"]
    auc_cols = ["endpoint", "horizon", "n", "n_events", "auc", "auc_lo", "auc_hi"]
    return (pd.DataFrame(err_rows, columns=None if err_rows else err_cols),
            pd.DataFrame(auc_rows, columns=None if auc_rows else auc_cols))
...
def _pick_examples(cache, C, n=4):
    ...
    Selection is made on COGNITIVE cells only (both series in a stage, not Death) so that a
...
def panel_C(fig, spec, cache, letter="c"):
    ...
    for ax, (i, tag) in zip(ex_axes, picks):
        ...
        ax.set_yticklabels(F2.STATE_NAMES, fontsize=7)
...
def compute_D(cache, n_neighbors=30, min_dist=0.3, seed=RNG_SEED):
    ...
    Purity is measured with k-NN in the ORIGINAL embedding space (64-d for this model, not in
    the 2-d projection), so it reports what the model actually encodes rather than what the
    projection happens to show. That separation is what makes the projection OPTIONAL: the
    claim is the purity number, the scatter is only for looking at. umap-learn is declared in
    environment.yml but is absent from both the local and the cluster environment, so rather
    than crash, this falls back to a deterministic 2-component PCA and SAYS SO -- in the panel
    title, in the axis labels, in the source CSV column names and in metrics.json. A figure
    that silently relabels PCA as UMAP would be worse than one without a scatter.
    ...
    if umap is not None:
        ...
        red = PCA(n_components=2, random_state=seed).fit_transform(Xb)
    ...
    # purity only (no projection) for the leaky one -- kNN in the raw space is cheap, the
    # projection is not, and the leaky arm is recorded rather than plotted anyway
    ...
    # structurally constrained by the baseline stage (a subject already at Severe cannot
    # "progress (stage)"; a Normal one cannot "improve"), and the embedding reads the baseline
    # stage almost perfectly. So most of the raw lift is just "it knows your current stage".
    # Restricting the neighbour search to subjects in the SAME baseline stage removes that path.
    # On RADC there is a second confounder the NACC version did not have: 6.5% of subjects
    # carry a 'Dementia at entry' static, which is in the baseline prompt, so the embedding can
    # read prevalent disease directly. That is background, not leakage -- but it is another
    # reason the raw purity overstates prognosis.
...
def panel_D(fig, spec, cache, letter="d"):
    ...
    def draw(ax, labels, palette, order, ttl, purity, ch, legend_title, ncol):
        ...
        ax.set_xlabel(f"{D['proj']} 1", fontsize=9)
        ax.set_ylabel(f"{D['proj']} 2", fontsize=9)
    ...
    metrics = dict(
        ...
        within_baseline_state=dict(
            note="neighbours restricted to the same baseline cognitive stage -- removes the path "
    ...
    tab = pd.DataFrame(dict(pid=cache["df"]["pid"].to_numpy(),
                            ...
                            **{f"{D['proj'].lower()}1_base": xy[:, 0],
                               f"{D['proj'].lower()}2_base": xy[:, 1]}))
    return metrics, tab
...
def supp_transition_matrices(cache):
    ...
    for ax, M, P, ttl in [(axes[0], obsM, rn(obsM), "observed (data)"),
        ...
        ax.set_xticks(range(F2.NSTATE)); ax.set_xticklabels(F2.STATE_NAMES, rotation=40, ha="right",
                                                            fontsize=8)
        ax.set_yticks(range(F2.NSTATE)); ax.set_yticklabels(F2.STATE_NAMES, fontsize=8)
        for i in range(F2.NSTATE):
            for j in range(F2.NSTATE):
                ...
                rows.append(dict(kind=ttl, frm=F2.STATE_NAMES[i], to=F2.STATE_NAMES[j],
...
def load_all(n_mc=F2.N_MC, limit=0, split=F2.SPLIT, cohort="matched", dataset=F2.DATASET,
    ...
    if cohort == "matched":
        ...
        info.update(n_in_cohort=int(sel.sum()), n_dropped=int((~sel).sum()),
                    rule=f"filter_cohort: >={F2.COHORT_MIN_VISITS} predicted-event ages OR "
                         f">={F2.COHORT_SHORT_MIN_VISITS} + an MMSE staging transition")
