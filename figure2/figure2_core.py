...
# --------------------------------------------------------------------------- configuration
CKPT = "out-radc-v3-final/ckpt.pt"             # the model under study
DATASET = "radc-v3-s42"
SPLIT = "test"
SEED = 42
N_MC = 100
...
#
# WHAT CHANGED FROM THE NACC VERSION. That one REPLICATED train.py's rule here, with a comment
# telling the reader to keep the two in sync -- and the NACC ids in this file were in fact one
# off from train.py's default at the time. This calls the real function instead, so the two can
# no longer disagree. `stage_disk` and `ignored_disk` come from vocab, which is where the RADC
# arm keeps every id.
COHORT_MIN_VISITS = 2
...
def training_cohort_mask(dataset=DATASET, split=SPLIT, min_visits=COHORT_MIN_VISITS,
    ...
    sets both thresholds to 2, and the measured effect on train was 3,101 -> 2,801 subjects
    (90.3% kept) against NACC's 38,687 -> 16,262 (42%). The matched/full distinction is
    ...
    return np.array([int(p2i[k, 0]) in starts for k in range(len(p2i))], dtype=bool)
