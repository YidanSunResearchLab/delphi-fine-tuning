...
def n_bins(name):
    if name in PD.SCALE_IDS:
        return len(PD.SCALE_IDS[name])
    return 2                                # a discrete event: absent / present
...
def at_risk_mask(df, name):
    ...
    Two exclusions, both structural rather than statistical:
      ...
      base_bin == top  the patient is ALREADY at the worst bin, so worsening is impossible.
                     ...
                     much larger on the coarse scales, so it is not a rounding error.
    """
...
def main():
    ...
    if a.cohort == "matched":
        ...
        # "baseline_state != final_state" as a stand-in for ">=2 staging tokens") would
