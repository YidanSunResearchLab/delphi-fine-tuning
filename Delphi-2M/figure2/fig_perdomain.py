...
Reads perdomain_<cohort>.json (written by score_perdomain.py) and renders:
  ...
  a  forest plot -- AUC at 5 y with a 95% patient-bootstrap CI, one row per outcome,
     grouped into instrument blocks (CDR / FAQ / NPI-Q / other) and sorted within each
...
FAMILY = {"CDR": PD.CDR_BOXES + ["CDRSUM_recon"],
          "FAQ": PD.FAQ_DOMAINS + ["FAQ_recon"],
          "NPI-Q": PD.NPI_SYMPTOMS + ["NPIQ_recon"],
          "other": ["GDS", "MOCA", "NACCUDSD"]}
# validated: node scripts/validate_palette.js "#0072B2,#009E73,#D55E00,#E69F00" --pairs all
FAM_COLOR = {"CDR": "#0072B2", "FAQ": "#009E73", "NPI-Q": "#D55E00", "other": "#E69F00"}
HORIZONS = [1, 2, 3, 5, 10]
PRIMARY_H = 5
